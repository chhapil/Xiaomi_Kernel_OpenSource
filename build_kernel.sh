...
KERNEL_CONFIG_FILE=gemini_user_defconfig
...
export CROSS_COMPILE=$KERNELDIR/android-toolchain/bin/aarch64-linux-android-;
	...
	cp arch/arm64/configs/"$KERNEL_CONFIG_FILE" .config
	...
	if [ -e "$KERNELDIR"/arch/arm64/boot/Image ]; then
		...
		#./mkbootimg --kernel "$kernel" --ramdisk "$ramdisk" $second --cmdline "$cmdline" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff $secondoff --tags_offset $tagsoff $dtb -o newboot.img;
                cp arch/arm64/boot/zImage AIK-Linux/split_img/boot.img-zImage
                cd AIK-Linux
